...
import {api_url, colors,fonts, get_KpHouseCusps, get_Kpplanets,getFontSize} from '../config/Constants';
...
const ShowKundliKpHouseCusp = props => {
              ...
              <TouchableOpacity onPress={() => toggleDropdown(item)} activeOpacity={1}>
                <View style={[styles.itmeContainer,{backgroundColor:calculateColor(item)}]}>
                  <Text style={styles.itemText}>
                    {nakshatra[item].house_id}
                    
                    </Text>
                  <Text style={styles.itemText}>{`${Math.floor(((Math.floor(nakshatra[item].cusp_full_degree))))}° ${Math.floor((nakshatra[item].cusp_full_degree % 1) * 60)}' ${Math.floor(((nakshatra[item].cusp_full_degree % 1) * 60) % 1 * 60)}"`}
</Text>
...
export default ShowKundliKpHouseCusp;
